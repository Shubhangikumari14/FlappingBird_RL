...
class Agent:
      ...
      self.epsilon.init=params["epsilon.init"]
            ...
            for episode in itertools.count():
                  ...
                  while (not terminated and episode_reward<self.reward_threshold):
                        if is_training and random.random()<epsilon:
                              ...
                              with torch.no_grad():
                                    action=policy_dqn(state.unsequeeze(dim=0)).squeeze().argmax()
      ...
      def optimize(self,mini_batch,policy_dqn,target_dqn):
             ...
             terminaions=torch.tensor(terminations).float().to(device)
             ...
             with torch.no_grad():
                              ...
                              target_q=reward+(1-terminations)*self.gamma*target_dqn(next_states).max(dim=1)[0]
